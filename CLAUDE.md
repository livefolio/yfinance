...
- **Don't pair `YfinanceDataFeed` with a live broker `Executor`** — bars are total-return-adjusted (Yahoo's `adjclose/close` ratio applied uniformly to OHL), which is correct for backtests but breaks limit-order semantics and historical re-scaling for live trading. For live, use the broker's own data feed (e.g. future `@livefolio/datafeed-alpaca` + `@livefolio/executor-alpaca`).
